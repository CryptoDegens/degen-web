...
const getYYCRVPrice = async function (App) {
    ...
    return YYVirtualPrice
}

const getYALINKVPrice = async function (App) {
    const prices = await lookUpPrices(["chainlink"]);
    const linkPrice = prices["chainlink"].usd
    const linkPerYlink = 1.030414
    return linkPrice * linkPerYlink
}

const getYYFIVPrice = async function (App) {
    const YEARN_VAULT_CONTROLLER = new ethers.Contract(YEARN_VAULT_CONTROLLER_ADDR, YEARN_VAULT_CONTROLLER_ABI, App.provider);
    const vaultAddress = await YEARN_VAULT_CONTROLLER.vaults(YFI_TOKEN_ADDR);
    const vaultContract = new ethers.Contract(vaultAddress, YEARN_VAULT_ABI, App.provider);
    ...
    return yfiPrice * currentPricePerFullShare
}

const getYUSDCVPrice = async function (App) {
    const YEARN_VAULT_CONTROLLER = new ethers.Contract(YEARN_VAULT_CONTROLLER_ADDR, YEARN_VAULT_CONTROLLER_ABI, App.provider);
    const vaultAddress = await YEARN_VAULT_CONTROLLER.vaults(USDC_TOKEN_ADDR);
    const vaultContract = new ethers.Contract(vaultAddress, YEARN_VAULT_ABI, App.provider);
    const currentPricePerFullShare = await vaultContract.getPricePerFullShare() / 1e18;
