...
async function main() {
    ...
    const stakingTokenAddr = DEGEN_YYCRV_UNI_TOKEN_ADDR;
    const stakingTokenTicker = "UNIV2";
    const rewardPoolAddr = DEGEN_YYCRV_REWARD_ADDR;
    ...
    const DEGEN_TOKEN = new ethers.Contract(DEGEN_TOKEN_ADDR, ERC20_ABI, App.provider);
    ...
    const totalYCRVInUniswapPair = await YYCRV_TOKEN.balanceOf(DEGEN_YYCRV_UNI_TOKEN_ADDR) / 1e18;
    const totalYAMInUniswapPair = await DEGEN_TOKEN.balanceOf(DEGEN_YYCRV_UNI_TOKEN_ADDR) / 1e18;
