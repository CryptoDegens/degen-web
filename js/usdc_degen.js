...
async function main() {
    ...
    const stakingTokenAddr = DEGEN_USDC_DEGEN_UNI_TOKEN_ADDR;
    const stakingTokenTicker = "UNIV2";
    const pairTokenAddr = USDC_TOKEN_ADDR;
    const pairTokenTicker = "USDC";
    const rewardPoolAddr = DEGEN_USDC_DEGEN_REWARD_ADDR;
    const rewardTokenAddr = DEGEN_TOKEN_ADDR;
    const rewardTokenTicker = "DEGEN";
    const prices = await lookUpPrices(["usd-coin", "spaghetti"]);
    const pairTokenPrice = prices["usd-coin"].usd;
