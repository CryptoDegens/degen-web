...
async function main() {
    ...
    const stakingTokenAddr = DEGEN_YFI_DEGEN_UNI_TOKEN_ADDR;
    const stakingTokenTicker = "UNIV2";
    const pairTokenAddr = YFI_TOKEN_ADDR;
    const pairTokenTicker = "YFI";
    const rewardPoolAddr = DEGEN_YFI_DEGEN_REWARD_ADDR;
    const rewardTokenAddr = DEGEN_TOKEN_ADDR;
    const rewardTokenTicker = "DEGEN";
    const prices = await lookUpPrices(["yearn-finance", "spaghetti"]);
    const pairTokenPrice = prices["yearn-finance"].usd;
    ...
    const totalSupplyOfStakingToken = await stakingToken.totalSupply() / 1e18;
    const totalStakedYAmount = await stakingToken.balanceOf(rewardPoolAddr) / 1e18;
    const totalPariTokenInUniswapPair = await pairToken.balanceOf(stakingTokenAddr) / 1e18;
    const totalRewardTokenInUniswapPair = await rewardToken.balanceOf(stakingTokenAddr) / 1e18;
    const stakingTokenPrice = (totalRewardTokenInUniswapPair * rewardTokenPrice + totalPariTokenInUniswapPair * pairTokenPrice) / totalSupplyOfStakingToken;
    ...
    _print(`1 ${pairTokenTicker}  = $${pairTokenPrice}`);
