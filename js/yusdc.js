...
async function main() {
    ...
    const stakingTokenAddr = YUSDC_TOKEN_ADDR;
    const stakingTokenTicker = "YUSDC";
    const rewardPoolAddr = YUSDC_REWARD_ADDR;
    const rewardTokenAddr = DEGEN_TOKEN_ADDR;
    const rewardTokenTicker = "DEGEN";
    const prices = await lookUpPrices(["spaghetti"]);
    const stakingTokenPrice = await getYUSDCVPrice(App)
