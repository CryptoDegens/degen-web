...
async function main() {
    ...
    const stakingTokenAddr = YYFI_TOKEN_ADDR;
    const stakingTokenTicker = "YYFI";
    const rewardPoolAddr = YYFI_REWARD_ADDR;
    const rewardTokenAddr = DEGEN_TOKEN_ADDR;
    const rewardTokenTicker = "DEGEN";
    const prices = await lookUpPrices(["spaghetti"]);
    const stakingTokenPrice = await getYYFIVPrice(App)
